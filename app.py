...
if uploaded_image is not None:
    # Display the uploaded image in the app so the user can verify it
    ...
    if st.button("Extract and Process Handwritten Notes"):
        with st.spinner("Reading handwriting and structuring data..."):
            try:
                # Read the image bytes for the API call
                image_bytes = uploaded_image.getvalue()
                
                # Encode the image to base64 so OpenAI's vision model can read it
                import base64
